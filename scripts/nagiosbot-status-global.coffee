...
outputResponse = (robot, messageObject, user, room, hostResponse, serviceResponse) ->
  ...
  message = message + " ".repeat(paddingLength + 3) + "#{utils.padToMatchLength(hostsHeader[0],totalHosts)}/#{utils.padToMatchLength(hostsHeader[1],hostsUp)}/#{utils.padToMatchLength(hostsHeader[2], hostsWarning)}/#{utils.padToMatchLength(hostsHeader[3], hostsDown)}\n"
  message = message + " ".repeat(paddingLength) + servicesHeader.join("/") + "\n"
  message = message + " ".repeat(paddingLength + 3) + "#{utils.padToMatchLength(servicesHeader[0],totalServices, offset=-3)}/#{utils.padToMatchLength(servicesHeader[1],servicesUp)}/#{utils.padToMatchLength(servicesHeader[2], servicesWarning)}/#{utils.padToMatchLength(servicesHeader[3], servicesDown)}\n"
  if room != "Shell"
    message = "```" + message + "```"
